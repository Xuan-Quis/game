...
public class Monster extends Character {
    ...
    public Monster(PointF pos) {
        ...
        setStartHealth(140); // chỉnh HP cho monster nếu muốn
    }

    /**
     * Cập nhật đơn giản (nếu không cần target player).
     * Giữ tương thích với Skeleton.update(delta, gameMap).
     */
    public void update(double delta, GameMap gameMap) {
        ...
        if (preparingAttack) checkTimeToAttackTimer();
        if (attacking) updateAttackTimer();
    }

    /**
     * Chuẩn bị tấn công: dừng di chuyển, xoay mặt về phía player, set timer.
     */
    ...
    /**
     * Cập nhật với target player + camera (dùng khi quái có thể đuổi player)
     */
    ...
    private float getDistanceToPlayer(Player player, float cameraX, float cameraY) {
        ...
        float monsterX = hitbox.left;
        float monsterY = hitbox.top;
    ...
    // Getters / setters + helpers used by Playing.java

    public boolean isPreparingAttack() {
        return preparingAttack;
    }

    public void setMonsterInactive() {
        active = false;
    }
